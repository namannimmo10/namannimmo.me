...
scale of petabyte-sized datasets -- inevitably most datasets at Google's scale.
...
Notice in particular that each of these categories of data usually involve
*nested* data. Further, the paper makes the observation that if you *were* to
store such data using a relational model, the process of de-structuring, or
"unnesting", and re-structuring this data, as well as splitting it into well-[normalized](https://en.wikipedia.org/wiki/Database_normalization)
database tables, is __prohibitive__ at web scale.

So, what if we could ...

* Store such data in its original, nested structure,
* Access data *in-situ* (as-is) without re-structuring it,
...
* Fields or sub-records (*groups*) can be:
  ...
  * repeated (occurs *zero* or more times).

In particular, there is an important connection between *optional* and
*repeated* fields in that both can be omitted. *Omitted* here means that it need
not have a value when serializing a concrete object from some programming
language into the final data format (which can then be stored, or sent over the
wire). This will become very relevant in the later discussion on lossless
...
In the paper, the top-most field name is often omitted, e.g.
...
This is called the *storage model*. For this, Dremel borrows from the world of
*Online Analytical Processing* (OLAP) relational databases, which for many years
now have exploited the benefits of *column-oriented* storage. The idea of
column-oriented storage in the world of relational databases is very simple:
Instead of storing tuples "row after row" in memory, data is stored "column
after column". For this table:
...
The latter is beneficial to queries that do not read entire rows at a time, but
access only specific columns. It turns out that most data analysis workloads are
like this. For example, the following "business intelligence" analysis
...
needs, while in row-oriented storage it (naively) would need to access every
single row.
...
decoding of data. It's noteworthy to mention what column-oriented storage
...
would be `Document.DocId` or `Document.Links.Backward`. Note that `group`s
aren't stored themselves, since they exclusively influence the logical structure
of the data but do not store data themselves. Only a leaf field can actually
have *value*.
...
structure. However, now values are stored side-by-side, flat. Upon re-assembly,
how do we know that the first two values belong to the first `Name` group, and
the third value to the second `Name` group?
...
For the value 'en-us', it was the `Language` group that repeated "most
recently". For the 'en-gb' value, it was the entire `Name` group that repeated
most recently. As such, the *repetition level* of the latter value is 1, because
in the field path `Name.Language.Code` it is the first field, `Name`, that was
...
Now, equipped with the power of assigning a repetition level to each value we
store, we can go back and amend our in-memory layout from above to the
following:
...
how much of the path is omitted.

##### Storage of Repetition and Definition Level

The paper mentions a number of optimizations that are used to store definition
and repetition levels efficiently. As mentioned above, one such optimization is
to omit explicit `NULL` values as the definition level already reveals whether a
value is present in the final record or not. Further, definition levels are not
stored for required fields, since by the nature of that qualification the field
and thus its whole path must be defined. Also, if the definition level is zero,
the repetition level must be zero too because the whole group (one level below
the whole record, e.g. `Document`) is omitted. Levels in general stored in a
bit-packed fashion.

#### Assembly of Records

As we'll review in more detail later, Dremel emits records as its query output.
To so, it must assemble records from the data columns it reads from memory. What
is particularly important and interesting about this task is that Dremel must be
able to read *partial* records. That is, records that follow the structure of
the record schema (the Protocol Buffer specification) while omitting certain
fields or entire groups. The paper describes that at Google many datasets are
*sparse*, with thousands of fields in the record schema, of which usually only a
small subset is queried. Most of the examples we've looked at so far were
partial records, for example in
...
Nevertheless, the record follows the original schema, with `Code` nested under
`Language` and `Language` under `Name`.
...
This FSM constructs the (partial) record by logically jumping from column to
column and scanning values from memory until exhausted. Edge values indicate
repetition levels.
...
referenced by their full, nested path. In (normalized) relational databases,
there is only one level of nesting, so this is a noticeable difference. Here is
an example query:
...
In all generality, this query language takes a table -- a collection of records
...
given a record substructure, or subtree, the `WHERE` statement can have the
effect of taking the entire tree out of consideration. Above, if a particular
`Name` substructure of a `Document` record does not have a `Url` field matching
the `^http` regular expression, the whole `Name` substructure is thrown away.
This means its `Name.Language.Code` field, albeit a different field from
`Name.Url`, will not be used to produce as record even though it is used in the
projection (second line). The second clause in the selection, `DocId < 20`,
prunes away entire records (`Documents`, at the top-most level).
...
`Name.Language.Code` values for one `Name.Url`. The semantics of Dremel here
dictate that one value will be produced for each value at the most-nested level,
i.e. one value for every `Name.Language.Code`, potentially using the same
`Name.Url`. This is exemplified by the sample output shown above, which could
have been produced by a record like this:
...
Dremel's *aggregation* semantics are showcased by the following line in the above query:
...
computing. One concept from this realm is that of a *serving tree*. In
particular, the way Dremel executes a query is that it first sends it from the
client (e.g. the web interface a data analyst uses to write her query) to a
*root server*. The root server then reads metadata about the tables the query
accesses. For clarification, a *table* here means a collection of records. This
root server then forwards a rewritten version of the query to a number of
intermediary servers below it in the serving tree, which may, recursively, do
the same. This process repeats until the (rewritten) query reaches *leaf
servers*. Leaf servers are the servers that have a direct link to the storage
layer in which data is located, such as a GFS cluster. Each leaf server has
access to a subset of the entire table.
...
all the way to the root server. This follows the divide-and-conquer pattern
...
smaller and smaller queries it that the final amount of work to be done by leaf
servers is ususally quite small. This allows the entire query to be executed
faster than if the root server had to scan the entire table sequentially itself.
One experiment in the paper actually shows that changing the serving tree
topology from 1:2900 (one root server, 2900 leaf servers) to 1:100:2900 (one
root server, 100 intermediate servers, 2900 leaf servers) results in an order of
magnitude performance gain for particular queries.

### Query Dispatch

Finally, the paper touches upon interesting details about how Dremel approaches
*query dispatch*. Specifically, this topic deals with how Dremel deals with
...
the `Name.Language.Code` column. The paper lists 100K-800K as typical examples
...
query is considered copmlete and a result is returned. It turns out that
reducing this parameter from 100% to something slightly lower, like 98%, can
result in "signficant" speedups to the overall execution.
...
Datasets". What can we take away from it? The paper says:
